...
import { generateFrameworkAgreementPDF } from '../utils/pdfGenerator';
import {
  createFrameworkAgreement,
  getFrameworkAgreementById,
  listFrameworkAgreementsByStatus,
...
const getFrameworkFrontendBaseUrl = (): string => {
  return (
...
const generateFrameworkToken = (): string => {
  return `fa_${uuidv4().replace(/-/g, '')}`;
};

const generateSignedFrameworkPdfHtml = (agreementData: any): string => {
  ...
  // Build transaction fees table
  const feeTableRows = transactionFeeTiers.map((tier: any) => `
    <tr>
  ...
  const contactsTableRows = primaryContacts.map((contact: any) => `
    <tr>
  ...
  // Build bank accounts table
  const bankAccountsRows = bankAccounts.map((account: any) => `
    <tr>
...
const generateSignedFrameworkPdfBuffer = async (agreementData: any): Promise<Buffer> => {
...
router.post('/framework-agreement', async (req, res) => {
  try {
    ...
    const buffer = await generateFrameworkAgreementPDF(payload);
...
router.post('/framework-agreement/email', async (req, res) => {
  try {
    ...
    await createFrameworkAgreement(frameworkAgreement);
...
<body>
  <div class="card">
    ...
    await updateFrameworkAgreement(agreementId, {
      emailSent: true,
...
router.get('/framework-agreement/signed', async (req, res) => {
  try {
    ...
    const signedFrameworks = (await listFrameworkAgreementsByStatus('acknowledged')).slice(0, limitNum);
...
router.get('/framework-agreement/:token', async (req, res) => {
  try {
    ...
    const agreement = await getFrameworkAgreementById(token);
    ...
    const updatedAgreement = await updateFrameworkAgreement(token, {
...
router.post('/framework-agreement/:token/sign', async (req, res) => {
  try {
    ...
    try {
      ...
      const uploadedDoc = await uploadDocumentToS3({
        folder: `framework-agreement/${effectiveAgreement.agreementId}`,
        fileName: signedFileName,
        contentType: 'application/pdf',
        body: signedPdfBuffer
      });
      await updateFrameworkAgreement(effectiveAgreement.agreementId, {
...
router.get('/framework-agreement/:token/pdf', async (req, res) => {
  try {
    ...
    if (agreement.signedDocumentKey) {
      const fromS3 = await getDocumentFromS3(agreement.signedDocumentKey);
      ...
      try {
        ...
        await updateFrameworkAgreement(agreement.agreementId, {
          signedDocumentKey: uploadedDoc.key,
          signedDocumentFileName: signedFileName
        });
      } catch (uploadError) {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="${agreement.signedDocumentFileName || `FrameworkAgreement-${agreement.agreementId}-signed.pdf`}"`
    );
    res.send(pdfBuffer);
  } catch (error) {
