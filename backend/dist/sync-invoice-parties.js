...
const entitySchema = new mongoose_1.default.Schema({}, { strict: false });
const transactionSchema = new mongoose_1.default.Schema({}, { strict: false });
const EntityModel = mongoose_1.default.model('Entity', entitySchema);
...
const syncInvoiceParties = async () => {
    ...
    for (const fileName of invoiceFiles) {
        ...
        await EntityModel.updateOne({ entityId: buyerId }, { $setOnInsert: makeEntityPayload('buyer', buyerName, buyerId) }, { upsert: true });
        await EntityModel.updateOne({ entityId: supplierId }, { $setOnInsert: makeEntityPayload('supplier', supplierName, supplierId) }, { upsert: true });
        await TransactionModel.updateOne({ _id: transaction._id }, {
            $set: {
                buyerId,
                buyerName,
