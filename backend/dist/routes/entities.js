...
router.get('/', async (req, res) => {
    try {
        ...
        const entities = await schemas_1.EntityModel.find().sort({ createdAt: -1 });
        res.json({
            success: true,
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const entity = await schemas_1.EntityModel.findById(id);
        if (!entity) {
...
router.get('/buyers/list', async (req, res) => {
    try {
        ...
        const buyers = await schemas_1.EntityModel.find({ type: 'buyer' }).sort({ createdAt: -1 });
...
router.get('/suppliers/list', async (req, res) => {
    try {
        ...
        const suppliers = await schemas_1.EntityModel.find({ type: 'supplier' }).sort({ createdAt: -1 });
...
router.get('/suppliers/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const supplier = await schemas_1.EntityModel.findById(id).where('type').equals('supplier');
        if (!supplier) {
            return res.status(404).json({
                success: false,
...
router.post('/', upload.single('agreementFrameworkDocument'), async (req, res) => {
    try {
        ...
        const newEntity = new schemas_1.EntityModel({
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const updatedEntity = await schemas_1.EntityModel.findByIdAndUpdate(id, { ...updateData }, { new: true, runValidators: true });
        if (!updatedEntity) {
            return res.status(404).json({
                success: false,
                message: 'Entity not found'
...
router.put('/:id/limits', async (req, res) => {
    try {
        ...
        const updatedEntity = await schemas_1.EntityModel.findByIdAndUpdate(id, limitUpdates, { new: true, runValidators: true });
...
router.delete('/:id', async (req, res) => {
    try {
        ...
        const deletedEntity = await schemas_1.EntityModel.findByIdAndDelete(id);
        if (!deletedEntity) {
            return res.status(404).json({
                success: false,
                message: 'Entity not found'
