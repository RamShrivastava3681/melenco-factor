...
const generatePdfBuffer = async (reportName, payload) => {
    ...
    const isCurrencyField = (key) => /(amount|limit|utilized|available|fees|revenue|profit|credit)/i.test(key);
    ...
    const tableHeader = columns
        ...
        return `<th class="${alignClass}">${escapeHtml(titleize(header))}</th>`;
    })
        .join('');
    const groupedRows = [];
    if (rows.length > 0) {
...
const buildReportData = async (configuration) => {
    ...
    const transactions = await schemas_1.TransactionModel.find().sort({ createdAt: -1 }).lean();
...
router.get('/filter-options', async (req, res) => {
    try {
        const [transactions, entities] = await Promise.all([
            schemas_1.TransactionModel.find({}, { buyerName: 1, supplierName: 1, transactionType: 1, currency: 1 }).lean(),
            schemas_1.EntityModel.find({}, { name: 1, type: 1 }).lean()
        ]);
...
router.get('/transactions', async (req, res) => {
    try {
        const transactions = await schemas_1.TransactionModel.find().sort({ createdAt: -1 }).limit(100);
        res.json({
            success: true,
            data: {
                transactions,
...
router.get('/compliance', async (req, res) => {
    try {
        ...
        const entities = await schemas_1.EntityModel.countDocuments();
        res.json({
            success: true,
            data: {
                overview: {
...
router.get('/entities', async (req, res) => {
    try {
        const entities = await schemas_1.EntityModel.find().sort({ createdAt: -1 }).limit(50);
        const totalCount = await schemas_1.EntityModel.countDocuments();
        res.json({
            success: true,
            data: {
                entities,
                summary: {
                    ...
                    suppliers: await schemas_1.EntityModel.countDocuments({ type: 'supplier' }),
                    buyers: await schemas_1.EntityModel.countDocuments({ type: 'buyer' })
                },
