...
const getNOAPartyDetails = async (transaction) => {
    const [buyerEntity, supplierEntity] = await Promise.all([
        schemas_1.EntityModel.findOne({ entityId: transaction.buyerId }).lean(),
        schemas_1.EntityModel.findOne({ entityId: transaction.supplierId }).lean()
    ]);
    return {
        buyer: {
...
router.post('/send', async (req, res) => {
    try {
        ...
        const transaction = await schemas_1.TransactionModel.findOne({ transactionId });
        if (!transaction) {
        ...
        const newNOA = new schemas_1.NOAModel({
            noaId: token,
            transactionId,
            buyerEmail,
...
router.get('/signed', async (req, res) => {
    try {
        ...
        const signedNoas = await schemas_1.NOAModel.find({ status: 'acknowledged' })
...
router.get('/:token', async (req, res) => {
    try {
        ...
        const noaData = await schemas_1.NOAModel.findOne({ noaId: token });
        if (!noaData) {
            return res.status(404).json({
                success: false,
        ...
        await noaData.save();
        const transaction = await schemas_1.TransactionModel.findOne({ transactionId: noaData.transactionId });
...
router.get('/status/:transactionId', async (req, res) => {
    try {
        ...
        const noaData = await schemas_1.NOAModel.findOne({ transactionId });
        res.json({
            success: true,
            data: {
                transactionId,
