...
const parseListQueryParam = (input) => {
...
const parseDateRangeFilter = (dateRange) => {
...
const generatePdfBuffer = async (reportName, payload) => {
    ...
    const isCurrencyField = (key) => /(amount|limit|utilized|available|fees|revenue|profit|credit|^current$|days_)/i.test(key);
    const isDateField = (key) => /(date|at)$/i.test(key);
    const isNumericField = (key, value) => asNumber(value) !== null || isCurrencyField(key);
    const getStatusBadge = (value, field) => {
    ...
    if (rows.length > 0) {
        if (columns.includes('entityType')) {
            ...
            if (buyers.length)
                groupedRows.push({ label: 'Buyers', rows: buyers });
            if (suppliers.length)
                groupedRows.push({ label: 'Suppliers', rows: suppliers });
        }
            ...
            if (openRows.length)
                groupedRows.push({ label: 'Open', rows: openRows });
            if (closedRows.length)
                groupedRows.push({ label: 'Closed', rows: closedRows });
        }
        else {
...
const buildDownloadFile = async (execution, payload, format) => {
...
const buildReportData = async (configuration) => {
    ...
    const txFiltered = transactions.filter((tx) => {
        const baseDate = tx.invoiceDate ? new Date(tx.invoiceDate) : new Date(tx.createdAt);
...
exports.router.get('/templates', async (req, res) => {
    try {
...
exports.router.get('/configurations', async (req, res) => {
    try {
...
exports.router.post('/configurations', async (req, res) => {
    try {
        const body = req.body || {};
...
exports.router.get('/executions', async (req, res) => {
    try {
...
exports.router.post('/configurations/:reportId/execute', async (req, res) => {
...
exports.router.get('/executions/:executionId/download', async (req, res) => {
    try {
...
exports.router.get('/templates/:templateId/download', async (req, res) => {
    try {
        const { templateId } = req.params;
        const requestedFormat = String(req.query.format || 'excel').toLowerCase();
        const template = reportTemplates.find((t) => t.id === templateId);
        if (!template) {
            return res.status(404).json({ success: false, message: 'Report template not found' });
        }
        const outputFormat = template.supportedFormats.includes(requestedFormat)
            ? requestedFormat
            : (template.supportedFormats[0] || 'csv');
        const pseudoConfiguration = {
...
exports.router.get('/filter-options', async (req, res) => {
    try {
        const [transactions, entities] = await Promise.all([
            (0, dynamoRepository_1.listTransactions)(),
            (0, dynamoRepository_1.listEntities)()
        ]);
        ...
        for (const tx of transactions) {
            ...
            const transactionType = String(tx.transactionType || '').trim();
...
exports.router.get('/transactions', async (req, res) => {
    try {
        const transactions = (await (0, dynamoRepository_1.listTransactions)()).slice(0, 100);
        res.json({
            success: true,
            data: {
                transactions,
...
exports.router.get('/compliance', async (req, res) => {
    try {
        const totalTransactions = (await (0, dynamoRepository_1.listTransactions)()).length;
        const entities = (await (0, dynamoRepository_1.listEntities)()).length;
        res.json({
            success: true,
            data: {
                overview: {
...
exports.router.get('/risk', async (req, res) => {
    try {
        res.json({
            success: true,
            data: {
...
exports.router.get('/performance', async (req, res) => {
    try {
        const totalTransactions = (await (0, dynamoRepository_1.listTransactions)()).length;
        res.json({
            success: true,
...
exports.router.get('/financial', async (req, res) => {
    try {
        const totalFees = (await (0, dynamoRepository_1.listTransactions)()).reduce((sum, tx) => sum + (tx.feeAmount || 0), 0);
        res.json({
            success: true,
            data: {
...
exports.router.get('/entities', async (req, res) => {
    try {
        const allEntities = await (0, dynamoRepository_1.listEntities)();
