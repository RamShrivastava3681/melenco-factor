...
const findEntityByAnyId = async (id) => {
    return (0, dynamoRepository_1.getEntityById)(String(id || ''));
};
...
const calculateLateFees = async (transaction) => {
    ...
    try {
        ...
        const buyer = await (0, dynamoRepository_1.getEntityById)(buyerId);
...
router.get('/open-invoices', async (req, res) => {
    try {
        ...
        const openTransactions = (await (0, dynamoRepository_1.listTransactions)())
            .filter((transaction) => ['funded', 'partial_payment'].includes(String(transaction.status || '').toLowerCase()) &&
        ...
        const invoicesWithLateFees = await Promise.all(openTransactions.map(async (transaction) => {
            ...
            const totalAmountDue = remainingAmount + outstandingLateFees;
...
router.delete('/open-invoices/:invoiceId', async (req, res) => {
    try {
        ...
        const transaction = await (0, dynamoRepository_1.getTransactionById)(invoiceId);
        if (!transaction) {
        ...
        const deletedTransaction = await (0, dynamoRepository_1.deleteTransaction)(invoiceId);
...
router.post('/open-invoices/:invoiceId/payment', async (req, res) => {
    try {
        ...
        const totalLateFees = await calculateLateFees(transaction);
        const totalLateFeesPaid = (transaction.paymentHistory || []).reduce((sum, p) => sum + (p.lateFeesPaid || 0), 0);
        ...
        await (0, dynamoRepository_1.updateTransaction)(invoiceId, {
        ...
        const updatedTransaction = await (0, dynamoRepository_1.getTransactionById)(invoiceId);
...
router.get('/closed-invoices', async (req, res) => {
    try {
        ...
        const closedTransactions = (await (0, dynamoRepository_1.listTransactions)())
...
router.get('/open-invoices/:invoiceId/closure-report', async (req, res) => {
    try {
        ...
        const supplier = await (0, dynamoRepository_1.getEntityById)(String(transaction.supplierId || ''));
        const buyer = await (0, dynamoRepository_1.getEntityById)(String(transaction.buyerId || ''));
...
router.get('/supplier-summary', async (req, res) => {
    try {
        ...
        const allTransactionsDebug = await (0, dynamoRepository_1.listTransactions)();
        ...
        const suppliers = (await (0, dynamoRepository_1.listEntities)()).filter((entity) => entity.type === 'supplier');
        ...
        const supplierSummaries = await Promise.all(suppliers.map(async (supplier) => {
            ...
            const allTransactions = allTransactionsDebug.filter((transaction) => String(transaction.supplierId || '') === String(supplier.entityId));
            ...
            const transactions = allTransactions.filter((transaction) => String(transaction.status || '').toLowerCase() === 'approved' &&
...
router.get('/incoming-payments', async (req, res) => {
    try {
        ...
        const fundedTransactions = (await (0, dynamoRepository_1.listTransactions)())
            .filter((transaction) => ['funded', 'partial_payment', 'settled'].includes(String(transaction.status || '').toLowerCase()) &&
            Boolean(transaction.fundedAt) &&
...
router.get('/payouts', async (req, res) => {
    try {
        const recentTransactions = (await (0, dynamoRepository_1.listTransactions)())
...
router.post('/payout', async (req, res) => {
    try {
        ...
        const supplier = await (0, dynamoRepository_1.getEntityById)(String(supplierId || ''));
        if (!supplier) {
            return res.status(404).json({
                success: false,
        ...
        const updateResult = await (0, dynamoRepository_1.updateTransactionsByIds)(transactionIds, {
            status: 'funded',
        ...
        const payoutRecord = await (0, dynamoRepository_1.createPayoutRecord)({
            payoutId,
            supplierId,
            supplierName: supplier.name,
            amount,
...
router.get('/payout-history', async (req, res) => {
    try {
        const payoutRecords = (await (0, dynamoRepository_1.listPayoutRecords)()).slice(0, 50);
...
router.get('/payout/:payoutId/acknowledgement', async (req, res) => {
    try {
        ...
        const payoutRecord = await (0, dynamoRepository_1.getPayoutRecordById)(payoutId);
        if (!payoutRecord) {
...
router.post('/pay-reserve', async (req, res) => {
    try {
        ...
        const transaction = await (0, dynamoRepository_1.getTransactionById)(incomingPaymentId);
        if (!transaction) {
            return res.status(404).json({
                success: false,
        ...
        const reservePayoutRecord = await (0, dynamoRepository_1.createPayoutRecord)({
        ...
        await (0, dynamoRepository_1.updateTransaction)(incomingPaymentId, {
            status: 'completed',
            completedAt: new Date().toISOString(),
